...
function isoToJerusalem(isoStr) {
...
exports.calendar = (0, https_1.onRequest)({ cors: true, region: 'europe-west1' }, async (req, res) => {
    const layerId = req.query['layer'];
    const classId = req.query['class'];
    const visParm = req.query['visibility'] ?? 'public';
    try {
        ...
        const filtered = snap.docs
            .map(d => d.data())
            .filter(ev => {
            if (layerId && !(ev['layerIds'] ?? []).includes(layerId))
                return false;
            if (classId && !(ev['classIds'] ?? []).includes(classId))
                return false;
            return true;
        });
